...
bool System_Task_Delete(task_new *task, char task_information[][FILE_LINE_LENGTH]);
void Task_Table(task_new *task);
void System_Task_Sort(task_new *tasks, int left, int right, char task_information[][FILE_LINE_LENGTH]);
bool Task_Edit(task_new *task);
void System_Task_Search(task_new *task);
void File_Information_Sort(int *data_type_sort); 
bool File_Information_Break(char task_information[][FILE_LINE_LENGTH], int type_index, int task_index, char* task_write_file);
bool System_File_Read(FILE **data_csv, task_new *task, char task_information[][FILE_LINE_LENGTH]);
bool System_File_Write(FILE **data_csv, task_new *task, char task_information[][FILE_LINE_LENGTH]);
int main(void);
...
void File_Information_Sort(int *data_type_sort)
{
    ...
    if (fgets(line_head, FILE_LINE_LENGTH, data_csv) != NULL)
    {
        ...
        if (strlen(line_head) >= 3 && 
            (unsigned char)line_head[0] == 0xEF && 
        ...
        for (int j = 0; j < 6; j++)
        {
            switch (type_sort[j])
            {
                ...
                case 6:
                    ...
                    {
                        check = File_Information_Break(task_information, type_sort[j], i, task_write_file);
                        if(!check)
                        {
                            return false;
                        }
